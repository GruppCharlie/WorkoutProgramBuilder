...
function initializeWorkoutFilters(config) {
    ...
    function applyFilters() {
        applyWorkoutFilters(gridId, activeFilters, emptyIcon, emptyMessage, emptySubtext);
    ...
    // Muscle checkboxes
    document.querySelectorAll('.muscle-checkbox, .muscle-filter').forEach(checkbox => {
        checkbox.addEventListener('change', (e) => {
            const muscle = e.target.value;
            if (e.target.checked) {
                activeFilters.muscles.push(muscle);
            } else {
                activeFilters.muscles = activeFilters.muscles.filter(m => m !== muscle);
            }
            applyFilters();
        });
    });
    ...
    document.querySelectorAll('.equipment-checkbox, .equipment-filter').forEach(checkbox => {
        checkbox.addEventListener('change', (e) => {
            const equipment = e.target.value;
            if (e.target.checked) {
                activeFilters.equipment.push(equipment);
            } else {
                activeFilters.equipment = activeFilters.equipment.filter(eq => eq !== equipment);
            }
            applyFilters();
        });
    });
    ...
    window.removeFilter = function(type, value) {
        if (type === 'muscle') {
            ...
            const checkbox = document.querySelector(`.muscle-filter[value="${value}"], .muscle-checkbox[value="${value}"]`);
            if (checkbox) checkbox.checked = false;
        } else if (type === 'equipment') {
            activeFilters.equipment = activeFilters.equipment.filter(e => e !== value);
            const checkbox = document.querySelector(`.equipment-filter[value="${value}"], .equipment-checkbox[value="${value}"]`);
            if (checkbox) checkbox.checked = false;
        }
        applyFilters();
    };
